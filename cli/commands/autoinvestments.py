"""
Autoinvestments command for querying autoinvestment details from the database
"""
...
class AutoinvestmentsCommand:
    """Command for querying autoinvestment details from the database"""
    
    @staticmethod
    def add_parser(subparsers):
        """Add autoinvestments subcommand to parser"""
        autoinvestments_parser = subparsers.add_parser(
            'autoinvestments',
            help='Query autoinvestment details from database',
            description='Query autoinvestment information from the database using user ID',
            formatter_class=argparse.RawDescriptionHelpFormatter,
            epilog="""
Examples:
  %(prog)s autoinvestments --user-id 6654
  %(prog)s autoinvestments --user-id 6654 --format json
  %(prog)s autoinvestments --user-id 6654 --status active
  %(prog)s autoinvestments --user-id 6654 --output autoinvestments.json
            """
        )
        
        # Search criteria
        autoinvestments_parser.add_argument(
            '--user-id',
            required=True,
            help='User ID to query autoinvestments for'
        )
        
        # Filter options
        autoinvestments_parser.add_argument(
            '--status',
            help='Filter by status (e.g., active, inactive, pending)'
        )
        
        autoinvestments_parser.add_argument(
            '--frequency',
            help='Filter by frequency (e.g., weekly, monthly)'
        )
        
        # Query options
        autoinvestments_parser.add_argument(
            '--table',
            default='mainpage_autoinvestments',
            help='Database table name (default: mainpage_autoinvestments)'
        )
    ...
    @staticmethod
    def execute(args):
        """Execute the autoinvestments command"""
        if args.no_color:
            Colors.disable()
        
        try:
            # Run the async query
            return asyncio.run(AutoinvestmentsCommand._execute_async(args))
    ...
    async def _execute_async(args):
        """Execute the autoinvestments command asynchronously"""
        try:
            # Load configuration
            config = AutoinvestmentsCommand._load_config(args.config)
            
            # Build SQL query
            sql_query, params = AutoinvestmentsCommand._build_query(args)
            ...
            # Execute query
            results = await AutoinvestmentsCommand._execute_query(config, sql_query, params)
            
            # Format and output results
            AutoinvestmentsCommand._output_results(results, args)
    ...
    def _build_query(args) -> tuple[str, list]:
        ...
        if args.frequency:
            where_conditions.append(f"frequency = ${param_count}")
    ...
    def _output_results(results: List[Dict[str, Any]], args):
        ...
        if not results:
            print(f"{Colors.YELLOW}[WARNING]{Colors.NC} No autoinvestments found")
            return
        
        if args.format == 'json':
            output = AutoinvestmentsCommand._format_json(results)
        elif args.format == 'csv':
            output = AutoinvestmentsCommand._format_csv(results)
        else:  # table
            output = AutoinvestmentsCommand._format_table(results)
