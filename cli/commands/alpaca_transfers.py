"""
Alpaca transfers command for querying transfer details from Alpaca Broker API
"""
...
class AlpacaTransfersCommand:
    """Command for querying transfer details from Alpaca Broker API"""
    
    @staticmethod
    def add_parser(subparsers):
        """Add alpaca-transfers subcommand to parser"""
        alpaca_transfers_parser = subparsers.add_parser(
            'alpaca-transfers',
            help='Query transfer details from Alpaca Broker API',
            description='Query transfer information from Alpaca Broker API using account ID',
            formatter_class=argparse.RawDescriptionHelpFormatter,
            epilog="""
Examples:
  %(prog)s alpaca-transfers --account-id 56846fba-222e-4dd3-b124-622c8b3d4f53
  %(prog)s alpaca-transfers --account-id 56846fba-222e-4dd3-b124-622c8b3d4f53 --format json
  %(prog)s alpaca-transfers --account-id 56846fba-222e-4dd3-b124-622c8b3d4f53 --output transfers.json
            """
        )
        
        # Required arguments
        alpaca_transfers_parser.add_argument(
            '--account-id',
            required=True,
            help='Alpaca account ID to query transfers for'
        )
    ...
    @staticmethod
    def execute(args):
        """Execute the alpaca-transfers command"""
        if args.no_color:
            Colors.disable()
        
        try:
            return AlpacaTransfersCommand._execute_request(args)
    ...
    def _execute_request(args):
        ...
        try:
            ...
            # Build request URL
            url = f"https://broker-api.alpaca.markets/v1/accounts/{args.account_id}/transfers"
            ...
            if response.status_code == 200:
                try:
                    data = response.json()
                    print(f"{Colors.GREEN}[SUCCESS]{Colors.NC} Retrieved {len(data) if isinstance(data, list) else 1} transfer(s)")
                    
                    # Format and output results
                    AlpacaTransfersCommand._output_results(data, args)
                    return 0
    ...
    def _output_results(data, args):
        ...
        if not data:
            print(f"{Colors.YELLOW}[WARNING]{Colors.NC} No transfers found")
            return
        
        if args.format == 'json':
            output = AlpacaTransfersCommand._format_json(data)
        else:  # table
            output = AlpacaTransfersCommand._format_table(data)
    ...
    def _format_table(data) -> str:
        ...
        # Get all field names from the first transfer
        first_transfer = transfers[0]
        if not isinstance(first_transfer, dict):
            return json.dumps(transfers, indent=2)
        
        fields = list(first_transfer.keys())
        ...
        for field in fields:
            widths[field] = max(
                len(str(field)),
                max(len(str(transfer.get(field, ''))) for transfer in transfers)
            )
        ...
        for transfer in transfers:
            row_str = " | ".join(
                f"{str(transfer.get(field, '')):<{widths[field]}}" 
                for field in fields
            )
